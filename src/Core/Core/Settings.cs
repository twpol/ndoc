...
namespace NDoc3.Core {
	...
	public class Settings : IDisposable {
		...
		/// </summary>
		public void Dispose() {
			SetSetting("versions", "lastSavedBy", Assembly.GetExecutingAssembly().GetName().Version.ToString());
		...
		private static string SerializeObject(object o) {
			Debug.Assert(!ReferenceEquals(o, null));
		...
		private XmlNode GetOrCreateSettingNode(string section, string name) {
			XmlNode sectionNode = data.SelectSingleNode(section) ?? data.AppendChild(data.OwnerDocument.CreateElement(section));

			XmlNode setting = sectionNode.SelectSingleNode(name) ??
			                  sectionNode.AppendChild(data.OwnerDocument.CreateElement(name));
		...
		private class NoPrologXmlWriter : XmlTextWriter {
			public NoPrologXmlWriter(TextWriter writer)
				: base(writer) {
			}
