...
namespace NDoc3.Core.Reflection {
	...
	internal class ExternalXmlSummaryCache {
		...
		private readonly string localizationLanguage;
		...
		public void AddXmlDoc(string xmlFileName) {
			...
			Debug.WriteLine("Cached doc : " + Path.GetFileName(xmlFileName) + " (" + ((Environment.TickCount - start) / 1000.0) + " sec.)");
		}
				...
				if (t != null) {
					...
					if (!String.IsNullOrEmpty(assemblyPath)) {
						...
						if ((docPath == null) || (!File.Exists(docPath))) {
							string frameworkPath = GetDotnetFrameworkPath(a.ImageRuntimeVersion);
		...
		private static void CleanupNodes(XmlNodeList nodes) {
		...
		/// <remarks>
		/// </remarks>
		private static void ProcessSeeLinks(string id, XmlNodeList nodes) {
			foreach (XmlNode node in nodes) {
				if (node.NodeType == XmlNodeType.Element) {
		...
		private static void MarkupSeeLinks(ref Hashtable linkTable, string id, XmlNode node) {
		...
		private static string GetDotnetFrameworkPath(string version) {
