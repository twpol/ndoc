...
namespace NDoc3.Core {
	...
	public class Workspace : IDisposable {
		...
		/// </summary>
		private readonly string cleanableFileTypes = string.Empty;
		...
		public void SaveOutputs(string filter) {
			...
			foreach (FileInfo f in dir.GetFiles(filter)) {
				string newFile = Path.Combine(RootDirectory, f.Name);
		...
		public void ImportProjectFiles(string sourceDirectory, string filter) {
			...
			foreach (FileInfo file in dir.GetFiles(filter))
				ImportFile(file, WorkingDirectory);
		}
		...
		private void ImportDirectory(DirectoryInfo sourceDir, DirectoryInfo targetParent) {
			...
			OnContentDirectoryAdded(GetRelativePath(new DirectoryInfo(WorkingDirectory), targetDir));

			foreach (FileInfo file in sourceDir.GetFiles())
				ImportFile(file, targetDir.FullName);
		...
		/// </summary>
		public void Clean() {
			string[] extenstions = cleanableFileTypes.Split(new [] { ';' });
			...
			foreach (string ext in extenstions) {
				try {
					string[] files = Directory.GetFiles(RootDirectory, ext);
