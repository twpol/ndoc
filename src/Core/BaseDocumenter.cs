	...
	abstract public class BaseDocumenter : IDocumenter, IComparable
	{
		...
		private void WriteNamespaces(XmlWriter writer, Module module)
		{
			...
			foreach (string namespaceName in namespaceNames)
			{
				...
				if (notEmptyNamespaces.ContainsKey(ourNamespaceName) || MyConfig.DocumentEmptyNamespaces)
				{
		...
		private void WriteConstructors(XmlWriter writer, Type type)
		{
			...
			if (MyConfig.DocumentInheritedMembers==DocumentedInheritedMembers.None)
			{
				bindingFlags = bindingFlags | BindingFlags.DeclaredOnly;
			}

			if (MyConfig.DocumentInheritedMembers==DocumentedInheritedMembers.InstanceAndStatic)
			{
