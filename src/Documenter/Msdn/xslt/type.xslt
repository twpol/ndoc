...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ndoc="urn:ndoc-schema"
      ...
      <xsl:choose>
        <xsl:when test="starts-with($list[$last]/@type, 'System.')">
          <a>
            ...
            <xsl:value-of select="NUtil:ToGeneralGenericFormat($list[$last]/@displayName)" />
          </a>
        </xsl:when>
        <xsl:otherwise>
  ...
  <xsl:template name="type">
    ...
    <html dir="LTR">
      <xsl:call-template name="html-head">
        <xsl:with-param name="title" select="concat(NUtil:ToGeneralGenericFormat(@displayName), ' ', $type)" />
      </xsl:call-template>
      <body id="bodyID" class="dtBODY">
        <xsl:call-template name="title-row">
          <xsl:with-param name="type-name" select="concat(NUtil:ToGeneralGenericFormat(@displayName), ' ', $type)" />
        </xsl:call-template>
        <div id="nstext">
            ...
            <xsl:if test="ndoc:constructor|ndoc:field|ndoc:property|ndoc:method|ndoc:operator|ndoc:event">
              <p>
                For a list of all members of this type, see <a href="{$members-href}">
                  <xsl:value-of select="NUtil:ToGeneralGenericFormat(@displayName)" /> Members
                </a>.
              </p>
            </xsl:if>
          </xsl:if>
  ...
  <xsl:template name="interface-implementing-types-section">
    <xsl:if test="ndoc:implementedBy">
      <h4 class="dtH4">
        Types that implement <xsl:value-of select="NUtil:ToGeneralGenericFormat(@displayName)" />
      </h4>
      <div class="tablediv">
