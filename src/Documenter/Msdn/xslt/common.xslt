...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:NUtil="urn:NDocUtil"
  ...
  exclude-result-prefixes="NUtil NHtmlProvider ndoc">
	...
	<xsl:template name="get-a-href">
		<xsl:param name="cref" />
		<xsl:variable name="href" select="string(NUtil:GetHRef(ancestor-or-self::ndoc:assembly/@name|@name, $cref))" />
		<xsl:choose>
			<xsl:when test="$href=''">
				<b>
					<xsl:value-of select="string(NUtil:GetName(ancestor-or-self::ndoc:assembly/@name|@name, $cref))" />
				</b>
			</xsl:when>
			<xsl:otherwise>
				<a>
	...
	<xsl:template name="get-a-href-with-name">
		<xsl:param name="cref" />
		<xsl:variable name="href" select="string(NUtil:GetHRef(ancestor-or-self::ndoc:assembly/@name, $cref))" />
		<xsl:choose>
			<xsl:when test="$href=''">
				<b>
					<xsl:value-of select="string(NUtil:GetName(ancestor-or-self::ndoc:assembly/@name, $cref))" />
				</b>
			</xsl:when>
			<xsl:otherwise>
				<a>
