...
namespace NDoc3.Gui {
	...
	public class BuildWorker {
		...
		private readonly IBuildStatus m_buildStatus;
		...
		public void Build(IDocumenter documenter, Project project) {
			...
			m_workerThread = new Thread(ThreadProc);
		...
		private void ThreadProc() {
			GC.Collect();
			Debug.WriteLine("Memory before build: " + GC.GetTotalMemory(false));

			try {
				m_documenter.DocBuildingStep += m_documenter_DocBuildingStep;
				...
				GC.Collect();
				Debug.WriteLine("Memory after build: " + GC.GetTotalMemory(false));
			}
		}
